...
(function(global) {
  ...
  const CONFIG = {
    serviceUrl: 'https://brtxgnn.yao.care/cds-hooks/',
    fhirBaseUrl: 'https://brtxgnn.yao.care/fhir/'
  };
  ...
  const SERVICES = {
    services: [
      ...
      {
        hook: 'order-sign',
        title: 'BrTxGNN 老藥新用候選',
        description: '基於 TxGNN 知識圖譜，提供病患用藥的老藥新用預測',
        id: 'brtxgnn-repurposing',
        prefetch: {
          patient: 'Patient/{{context.patientId}}',
          medications: 'MedicationRequest?patient={{context.patientId}}&status=active,completed',
  ...
  function processOrderSignDDI(request) {
    ...
    // Check for PDDI using BrTxGNN.PDDI if available
    if (global.BrTxGNN?.PDDI) {
      const allMeds = [...currentMeds, newMed];
      const pddiAlerts = global.BrTxGNN.PDDI.checkPDDI(allMeds);

      pddiAlerts.forEach(pddi => {
        const card = global.BrTxGNN.PDDI.generateCDSHooksCard(pddi);
        cards.push(card);
      });
    }

    // Also check with basic DDI checker
    if (global.BrTxGNN?.DDIChecker) {
      const ddiAlerts = global.BrTxGNN.DDIChecker.checkNewDrug(newMed, currentMeds);
  ...
  function processOrderSignRepurposing(request) {
    ...
    if (global.BrTxGNN?.DrugMapper) {
      ...
      cards.push({
        ...
        source: {
          label: 'BrTxGNN Drug Repurposing',
          url: 'https://brtxgnn.yao.care/'
        },
        links: [
          {
            label: '查看老藥新用預測',
            url: `https://brtxgnn.yao.care/smart/standalone.html?drugs=${encodeURIComponent(allMeds.join(','))}`,
            type: 'absolute'
          }
        ]
      });
  ...
  function processPatientView(request) {
    ...
    if (conditions.length > 0 || medications.length > 0) {
      cards.push({
        ...
        source: {
          label: 'BrTxGNN Clinical Trial Match',
          url: 'https://brtxgnn.yao.care/'
        },
        links: [
          {
          ...
          {
            label: '查看老藥新用預測',
            url: `https://brtxgnn.yao.care/smart/standalone.html?drugs=${encodeURIComponent(medications.join(','))}`,
            type: 'absolute'
          }
        ]
      });
  ...
  function demo(medications, conditions) {
    console.log('=== BrTxGNN CDS Hooks Demo ===');
    console.log('Medications:', medications);
    console.log('Conditions:', conditions || []);

    // Test DDI check
    const ddiRequest = createMockRequest('order-sign', medications, null);
    const ddiResponse = handleRequest('brtxgnn-ddi-check', ddiRequest);
    console.log('\n--- DDI Check Response ---');
    console.log(JSON.stringify(ddiResponse, null, 2));

    // Test repurposing
    const repurposingResponse = handleRequest('brtxgnn-repurposing', ddiRequest);
    ...
    if (conditions) {
      ...
      const trialResponse = handleRequest('brtxgnn-trial-match', trialRequest);
